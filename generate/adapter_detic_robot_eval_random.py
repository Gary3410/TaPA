...
def get_parser():
    ...
    parser.add_argument(
        "--navigation_strategy",
        default="Random_1_60",
        choices=['Random_1_60', 'Random_1_120', 'Random_75_60', 'Random_75_120'],
        help="",
    )
    parser.add_argument(
        "--base_data_path",
...
def main(
    ...
    for scene_one_name in scene_name_list:
        ...
        random_step = mode.split("_")[1]
        position_list = {}
        if random_step == "1":
            for k in random.sample(position_dict.keys(), 1):
                v = position_dict[k]
                position_list.update(dict({k: v}))
        elif random_step == "0.50":
            for k in random.sample(position_dict.keys(), math.ceil(len(position_dict) * 0.50)):
                v = position_dict[k]
                position_list.update(dict({k: v}))
        else:
            for k in random.sample(position_dict.keys(), math.ceil(len(position_dict) * 0.75)):
                v = position_dict[k]
                position_list.update(dict({k: v}))
